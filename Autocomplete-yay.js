...
function autocomplete(input, dictionary){
    let char = 'abcdefghijklmnopqrstuvwxyz'+'abcdefghijklmnopqrstuvwxyz'.toUpperCase()
    input = input.split('').filter(el=> char.includes(el)).join('')
    return dictionary.filter(word=> word.toLowerCase().indexOf(input.toLowerCase())==0).slice(0,5)
}