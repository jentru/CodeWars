...
VendingMachine.prototype.vend = function (selection, itemMoney){
    let sel = this.items.map(el=> el.code).indexOf(selection)
  
    if (sel<0){
      return `Invalid selection! : Money in vending machine = ${this.money.toFixed(2)}`
    } else if (this.items[sel].quantity ==0){
      return `${this.items[sel].name}: Out of stock!`
    } else if (itemMoney < this.items[sel].price){
      return 'Not enough money!'
    } else if (itemMoney > this.items[sel].price){
      this.money +=this.items[sel].price
      this.items[sel].quantity -=1
      return `Vending ${this.items[sel].name} with ${(itemMoney-this.items[sel].price).toFixed(2)} change.`
    } else if (itemMoney == this.items[sel].price){
      this.money +=this.items[sel].price
      this.items[sel].quantity -=1
      return `Vending ${this.items[sel].name}`
    }
