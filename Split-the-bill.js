...
function splitTheBill(x) {
    let avg = Math.round(Object.values(x).reduce((acc,cur)=>acc+cur,0)/Object.keys(x).length*100)/100
    Object.keys(x).forEach(person=> x[person]= Math.round((x[person]-avg)*100)/100)
  return x
}