...
function driver(data) {
    let res = ''
  //   first five
    res += data[2].slice(0,5).padEnd(5,'9').toUpperCase()
  //   6th
    res +=data[3].split('-')[2][2]
  //   7-8
    data[4]=='M'? res+= (new Date(data[3]).getMonth()+1).toString().padStart(2,'0') : res+= new Date(data[3]).getMonth()+1 +50
  //   9-10
    res+= new Date(data[3]).getDate().toString().padStart(2,'0')
  //   11
    res+= data[3].split('').reverse()[0]
  //   12-13
    res+= data[0][0]
    data[1]? res+= data[1][0]: res+= '9'
  //   14
    res+='9'
  //   15-16
    res+='AA'
