...
function fruit(reels, spins){
    ...
    let count ={}
    let result = [reels[0][spins[0]], reels[1][spins[1]], reels[2][spins[2]]]
    result.forEach(fruit=> count[fruit]? count[fruit]++ : count[fruit]=1)
    let fruits = Object.keys(count)
    let quantity = Object.values(count)
    let i = quantity.findIndex(el=> el==2)
    if (fruits.length==3){
      return 0
    } else if (fruits.length==1){
      return score[fruits]*10
    } else if (fruits[i]=='Wild'){
      return score[fruits[i]]
    } else if (fruits.includes('Wild')){
      return score[fruits[i]]*2
    } else {
      return score[fruits[i]]
    }
