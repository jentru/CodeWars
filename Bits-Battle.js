...
function bitsBattle(numbers) {
    let even =[], odd =[]
     numbers.map(n=> {
       (n%2==0 && n!==0)? even.push(n) : odd.push(n) })
     even = even.map(n=> n.toString(2).replace(/1/g,'')).map(n=> n.length).reduce((acc,cur)=> acc+cur,0) 
     odd = odd.map(n=> n.toString(2).replace(/0/g,'')).map(n=> n.length).reduce((acc,cur)=> acc+cur,0) 
