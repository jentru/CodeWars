...
function findTheKey(message, code)
{
  let key
  let temp
  let keyrep = message.split('').map((el,i)=> code[i]- (el.charCodeAt(0)-96)).join('')
  for (let i=1; i<= keyrep.length; i++){
    temp = keyrep.slice(0,i).repeat(keyrep.length).slice(0,keyrep.length)
    if(temp == keyrep){
      key = +keyrep.slice(0,i)
      break
    }
  }
  return key
