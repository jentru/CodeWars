...
function alphabetWar(fight)
{
  let left = ' sbpw'
  let right = ' zdqm'
  let res = fight.replace(/([a-z]?\*[a-z]?)/gi,'').split('').filter(el=> left.includes(el) || right.includes(el))
  res= res.reduce((acc,cur)=> left.includes(cur)? acc-left.indexOf(cur) : acc+right.indexOf(cur), 0)
